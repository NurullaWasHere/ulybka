    ...
    return (
        ...
        <div className={s.loginFone}>
            <div className={s.mainLogin}>
                <p className={s.tlogin}> Логин </p>
                    <form onSubmit={handleSubmit(onSubmit)}>

                        <p className={s.fill}>Заполните поля ниже</p>
                        <Input 
                        ...
                        />
                            {isError === "error" && <p className={s.dataError}>Ошибка данных</p>}
                            <button type="submit" className="border rounded-lg bg-gray-200 px-8 py-3 w-4/5 hover:bg-gray-300 transition duration-500 ease-in-out">Войти</button>
                </form>
            </div>

            {isValid === 'valid' && <p className={s.validData}>Успешно!</p>}
            </div>
        </>
    )
  }
